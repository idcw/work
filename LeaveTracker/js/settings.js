...
function load_category() {
	...
	if(cat_length > 0) {
		for (var i = 0; i < cat_length; i++) {
			row_list += "<tr id='cat_"+cat_list[i].id+"'>";
			row_list += "<td>"+(i+1)+"</td>";
			row_list += "<td id='cat_val_"+cat_list[i].id+"'>"+cat_list[i].name+"</td>";
			row_list += "<td id='cat_leave_"+cat_list[i].id+"'>"+cat_list[i].leave+"</td>";
			row_list += '<td id="cat_edit_'+cat_list[i].id+'"><a href="javascript:void(0);" onclick="return categ_update_form('+"'"+cat_list[i].id+"'"+');">Edit</a></td>';
			row_list += '<td><a href="javascript:void(0);" onclick="return delete_category('+"'"+cat_list[i].id+"'"+');">Delete</a></td>';
			row_list += "</tr>";
		};
	}
	else {
	...
	row_list += `
	<tr id="categ_add_more">
		<td colspan=5>
			<input type="button" value="Add" id="add_newc" onclick="return categ_add_form();">
		</td>
	</tr>`;
...
function add_category(category,annual_leave) {
	...
	if(category=='')
		...
		var categ = []
		categ.push(category)
		categ.push(annual_leave)
		// add the category to alasql
		id = alasql('SELECT MAX(id) + 1 as id FROM category')[0].id;
		categ.unshift(id);
		alasql(`INSERT INTO category(id, name, leave) VALUES(?,?,?);`,categ);
